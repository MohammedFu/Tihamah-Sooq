import type { ApiBanner, ApiCategory, ApiCommission, ApiListing, ApiRegion, ApiReport, ApiSystemSetting, ApiUser, ApiVillage } from "../../types/api";
import type { AdminAuditRecord } from "../../types/domain";

export const fixtureDate = "2026-09-05T08:00:00.000Z";
export interface AdminFixtureData {
  categories: ApiCategory[];
  regions: ApiRegion[];
  villages: ApiVillage[];
  banners: ApiBanner[];
  users: ApiUser[];
  listings: ApiListing[];
  commissions: ApiCommission[];
  reports: ApiReport[];
  settings: ApiSystemSetting[];
  audit: AdminAuditRecord[];
}
export function createAdminFixtureData(): AdminFixtureData {
  return {
    categories: ["سيارات ومركبات", "مواشي وحيوانات", "عقارات وأراضي", "إلكترونيات", "منتجات زراعية"].map((name, index) => ({ id: index + 1, name, icon_url: "", sort_order: index + 1, is_active: true })),
    regions: [{ id: 1, name: "جازان - تهامة", is_active: true }, { id: 2, name: "سهل تهامة", is_active: true }],
    villages: [{ id: 11, region_id: 1, name: "المضايا", is_active: true }, { id: 21, region_id: 2, name: "القوز", is_active: true }],
    banners: [{ id: 71, image_url: "https://cdn.example.test/banner.jpg", sort_order: 1, is_active: true }],
    users: [
      { id: 201, fullname: "فواز أبو عبدل", phone: "+966500000001", is_banned: false, region_id: 1, village_id: 11, created_at: fixtureDate },
      { id: 203, fullname: "أحمد علي", phone: "+966500000003", is_banned: true, ban_reason: "تكرار المخالفات", region_id: 2, village_id: 21, created_at: fixtureDate },
    ],
    listings: [{
      id: 1048,
      user_id: 201,
      user: { id: 201, fullname: "فواز أبو عبدل", phone: "+966500000001", is_banned: false, region_id: 1, village_id: 11, created_at: fixtureDate },
      category_id: 2,
      category: { id: 2, name: "مواشي وحيوانات", icon_url: "", sort_order: 2, is_active: true },
      region_id: 1,
      region: { id: 1, name: "جازان - تهامة", is_active: true },
      village_id: 11,
      village: { id: 11, region_id: 1, name: "المضايا", is_active: true },
      title: "مجموعة أغنام للبيع",
      description: "أغنام حري نظيفة ومطعمة بالكامل، المعاينة داخل القرية.",
      price: 15000,
      status: "pending_review",
      view_count: 0,
      media: [
        { id: 301, media_url: "https://cdn.example.test/sheep.jpg", media_type: "image", is_primary: true },
        { id: 302, media_url: "https://cdn.example.test/sheep.mp4", media_type: "video", is_primary: false },
      ],
      created_at: fixtureDate,
    }],
    commissions: [{
      id: 511,
      ad_id: 1048,
      ad: {
        id: 1048,
        user_id: 201,
        user: { id: 201, fullname: "فواز أبو عبدل", phone: "+966500000001", is_banned: false, region_id: 1, village_id: 11, created_at: fixtureDate },
        category_id: 2,
        category: { id: 2, name: "مواشي وحيوانات", icon_url: "", sort_order: 2, is_active: true },
        region_id: 1,
        region: { id: 1, name: "جازان - تهامة", is_active: true },
        village_id: 11,
        village: { id: 11, region_id: 1, name: "المضايا", is_active: true },
        title: "مجموعة أغنام للبيع",
        description: "أغنام حري نظيفة ومطعمة بالكامل، المعاينة داخل القرية.",
        price: 15000,
        status: "pending_review",
        view_count: 0,
        media: [],
        created_at: fixtureDate,
      },
      user_id: 201,
      user: { id: 201, fullname: "فواز أبو عبدل", phone: "+966500000001", is_banned: false, region_id: 1, village_id: 11, created_at: fixtureDate },
      amount: 150,
      status: "paid",
      transfer_receipt_url: "https://cdn.example.test/receipt.jpg",
      created_at: fixtureDate,
    }],
    reports: [{
      id: 801,
      ad_id: 1048,
      ad: {
        id: 1048,
        user_id: 201,
        user: { id: 201, fullname: "فواز أبو عبدل", phone: "+966500000001", is_banned: false, region_id: 1, village_id: 11, created_at: fixtureDate },
        category_id: 2,
        category: { id: 2, name: "مواشي وحيوانات", icon_url: "", sort_order: 2, is_active: true },
        region_id: 1,
        region: { id: 1, name: "جازان - تهامة", is_active: true },
        village_id: 11,
        village: { id: 11, region_id: 1, name: "المضايا", is_active: true },
        title: "مجموعة أغنام للبيع",
        description: "أغنام حري نظيفة ومطعمة بالكامل، المعاينة داخل القرية.",
        price: 15000,
        status: "pending_review",
        view_count: 0,
        media: [],
        created_at: fixtureDate,
      },
      reporter_id: 203,
      reporter: { id: 203, fullname: "أحمد علي", phone: "+966500000003", is_banned: false, region_id: 2, village_id: 21, created_at: fixtureDate },
      type: "misleading",
      reason: "الصورة لا تطابق الوصف",
      status: "open",
      created_at: fixtureDate,
    }],
    settings: [
      { id: 1, key: "commission_percentage", value: "1.0", description: "نسبة عمولة المنصة" },
      { id: 2, key: "is_otp_enabled", value: "false", description: "إرسال رمز التحقق" },
      { id: 3, key: "sms_provider", value: "taqnyat", description: "مزود بوابة الرسائل" },
      { id: 4, key: "sms_api_key", value: "fixture-secret-key", description: "مفتاح ربط بوابة الرسائل" },
      { id: 5, key: "sms_sender_name", value: "TIHAMAH", description: "اسم مرسل الرسائل" },
      { id: 6, key: "sms_username", value: "tihamah_sms", description: "اسم مستخدم بوابة الرسائل" },
      { id: 7, key: "sms_user_sender", value: "TIHAMAH", description: "معرّف مرسل المستخدم" },
    ],
    audit: [{ id: 9912, adminId: 1, admin: null, action: "VERIFY_COMMISSION", entityType: "commission", entityId: 508, metadata: null, ipAddress: "192.0.2.1", createdAt: fixtureDate }],
  };
}
