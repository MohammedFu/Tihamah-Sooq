    ...
    <>
      ...
      <section className="card data-surface">
        ...
        <div className="table-wrap"><table><thead><tr><th>الإعلان</th><th>المعلن</th><th>القسم والموقع</th><th>السعر</th><th>المشاهدات</th><th>الحالة</th><th>الإجراء</th></tr></thead><tbody>
          {visible.map((listing) => <tr key={listing.id}><td><div className="record-primary"><img src={listing.image} alt="" /><span><strong>{listing.title}</strong><small>#{listing.id} · {listing.createdAt}</small></span></div></td><td><strong>{listing.seller}</strong><small className="block-copy" dir="ltr">{listing.phone}</small></td><td>{listing.category}<small className="block-copy">{listing.village}، {listing.region}</small></td><td className="numeric">{listing.price.toLocaleString("ar-SA")} ر.س</td><td className="numeric">{listing.views.toLocaleString("ar-SA")}</td><td><StatusBadge value={listing.status} /></td><td><button className="icon-button table-action" type="button" onClick={() => setSelected(listing)} aria-label="عرض التفاصيل"><Eye size={17} /></button></td></tr>)}
        </tbody></table></div>
      ...
      <Drawer open={Boolean(selected)} title={selected ? `الإعلان #${selected.id}` : ""} onClose={() => setSelected(null)}>
        {selected && <div className="detail-stack">
          ...
          <div className="decision-actions">
            {selected.status === "pending_review" && <><button className="button success-button" type="button" onClick={() => updateStatus(selected.id, "active", "تم اعتماد الإعلان وإرساله للنشر")}><Check size={17} />اعتماد الإعلان</button><button className="button danger-outline" type="button" onClick={() => setRejecting(selected)}><X size={17} />رفض الإعلان</button></>}
            {selected.status === "active" && <button className="button secondary" type="button" onClick={() => updateStatus(selected.id, "rejected", "تم إلغاء تنشيط الإعلان")}><Pause size={17} />إلغاء التنشيط</button>}
            {selected.status === "rejected" && <button className="button secondary" type="button" onClick={() => updateStatus(selected.id, "active", "تمت إعادة تنشيط الإعلان")}><Play size={17} />إعادة التنشيط</button>}
            <button className="button danger-ghost" type="button" onClick={() => removeListing(selected)}><Trash2 size={17} />حذف نهائي</button>
          </div>
        </div>}
      </Drawer>
      <Modal open={Boolean(rejecting)} title="رفض الإعلان" onClose={() => setRejecting(null)}><label className="form-field"><span>سبب الرفض</span><select value={reason} onChange={(event) => setReason(event.target.value)}><option value="">اختر سبباً</option><option>سلعة ممنوعة</option><option>صور غير لائقة</option><option>سعر وهمي</option><option>بيانات الإعلان غير مكتملة</option></select></label><label className="form-field"><span>ملاحظات إضافية</span><textarea rows={4} placeholder="ستظهر هذه الملاحظات للمعلن" /></label><div className="modal-actions"><button className="button secondary" type="button" onClick={() => setRejecting(null)}>إلغاء</button><button className="button danger-button" type="button" disabled={!reason} onClick={reject}>رفض وإشعار المعلن</button></div></Modal>
      <Toast message={toast} />
    </>
  );
}
