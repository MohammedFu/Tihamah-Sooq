...
const auditColumns: DataTableColumn<AuditLog>[] = [
  { id: "id", header: "الرقم", className: "numeric", cell: (log) => <>#{log.id}</> },
  { id: "admin", header: "المشرف", cell: (log) => log.admin },
  { id: "action", header: "الإجراء", cell: (log) => <code className="action-code">{log.action}</code> },
  { id: "entity", header: "الكيان", cell: (log) => <code>{log.entity}</code> },
