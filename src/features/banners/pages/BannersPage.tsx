...
export function BannersPage() {
  ...
  return (
    <>
      <PageHeader title="البنرات الترويجية" description="إدارة صور الصفحة الرئيسية وروابطها وفترات ظهورها في تطبيق الموبايل." action={<AuthorizedButton resource="banners" action="create" className="button" type="button" onClick={() => openEditor("new")}><Plus size={17} />إضافة بنر</AuthorizedButton>} />
      ...
      <section className="banner-grid">{banners.map((banner, index) => <article className="card banner-card" key={banner.id}>
        ...
        <div className="banner-content"><div className="detail-title-row"><div><h2>{banner.title}</h2><p className="panel-copy">بنر <bdi dir="ltr">#{banner.id}</bdi></p></div><StatusBadge value={banner.isActive ? "enabled" : "disabled"} /></div><div className="banner-meta"><span><Link2 aria-hidden="true" size={15} />{targetLabels[banner.targetType]}: {banner.target}</span><span><CalendarDays aria-hidden="true" size={15} />{banner.startAt} - {banner.endAt}</span></div><div className="banner-actions"><AuthorizedButton resource="banners" action="edit" className="icon-button" type="button" disabled={index === 0} onClick={() => move(index, -1)} aria-label={`تحريك ${banner.title} للأعلى`}><ChevronUp aria-hidden="true" size={17} /></AuthorizedButton><AuthorizedButton resource="banners" action="edit" className="icon-button" type="button" disabled={index === banners.length - 1} onClick={() => move(index, 1)} aria-label={`تحريك ${banner.title} للأسفل`}><ChevronDown aria-hidden="true" size={17} /></AuthorizedButton><AuthorizedButton resource="banners" action="edit" className={`switch ${banner.isActive ? "on" : ""}`} type="button" role="switch" aria-checked={banner.isActive} onClick={() => toggle(banner.id)} aria-label={`تغيير حالة ${banner.title}`}><i /></AuthorizedButton><span className="spacer" /><AuthorizedButton resource="banners" action="edit" className="icon-button" type="button" onClick={() => openEditor(banner)} aria-label={`تعديل ${banner.title}`}><Pencil aria-hidden="true" size={16} /></AuthorizedButton><AuthorizedButton resource="banners" action="delete" className="icon-button danger-icon" type="button" onClick={() => remove(banner.id)} aria-label={`حذف ${banner.title}`}><Trash2 aria-hidden="true" size={16} /></AuthorizedButton></div></div>
      </article>)}</section>
