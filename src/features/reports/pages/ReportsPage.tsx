    ...
    <>
      ...
      <section className="card data-surface">
        ...
        <div className="table-wrap"><table><thead><tr><th>رقم البلاغ</th><th>النوع</th><th>المبلغ</th><th>الإعلان</th><th>المعلن عنه</th><th>الوقت</th><th>الحالة</th><th>الإجراء</th></tr></thead><tbody>
          {visible.map((report) => <tr key={report.id}><td className="numeric">#{report.id}</td><td><span className={`reason-chip ${report.type === "احتيال" ? "critical" : ""}`}><Flag size={13} />{report.type}</span></td><td>{report.reporter}</td><td>{report.listing}</td><td>{report.accused}</td><td>{report.createdAt}</td><td><StatusBadge value={report.status} /></td><td><AuthorizedButton resource="reports" action="show" className="icon-button table-action" type="button" onClick={() => setSelected(report)} aria-label="عرض البلاغ"><Eye size={17} /></AuthorizedButton></td></tr>)}
        </tbody></table></div>
      </section>
