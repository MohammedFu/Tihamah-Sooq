...
export function LocationsPage() {
  ...
  return (
    <>
      <PageHeader title="المناطق والقرى" description="إدارة الهيكل الجغرافي الذي يغذي التسجيل وفلاتر تطبيق الموبايل." action={<button className="button" type="button" onClick={() => openEditor({ type: "region" })}><Plus size={17} />إضافة منطقة</button>} />
      <div className="location-layout">
        <section className="card tree-panel">
          ...
          <div className="tree-list">{regions.map((region) => {
            ...
            return <article className="tree-region" key={region.id}><div className="tree-region-row"><button className="tree-toggle" type="button" onClick={() => setExpanded((ids) => ids.includes(region.id) ? ids.filter((id) => id !== region.id) : [...ids, region.id])}>{isExpanded ? <ChevronDown size={18} /> : <ChevronLeft size={18} />}<MapPin size={18} /><span><strong>{region.name}</strong><small>{region.villages.length} قرى</small></span></button><div className="row-actions"><StatusBadge value={region.isActive ? "enabled" : "disabled"} /><button className="icon-button table-action" type="button" onClick={() => openEditor({ type: "region", id: region.id, name: region.name })} aria-label="تعديل المنطقة"><Pencil size={15} /></button><button className="icon-button table-action" type="button" onClick={() => openEditor({ type: "village", regionId: region.id })} aria-label="إضافة قرية"><Plus size={16} /></button></div></div>
              {isExpanded && <div className="village-list">{region.villages.map((village, index) => <div className="village-row" key={village.id}><span className="order-index">{index + 1}</span><span className="village-name"><strong>{village.name}</strong><small>{village.activeListings} إعلان نشط</small></span><StatusBadge value={village.isActive ? "enabled" : "disabled"} /><div className="row-actions"><button className="icon-button table-action" type="button" onClick={() => openEditor({ type: "village", regionId: region.id, id: village.id, name: village.name })} aria-label="تعديل القرية"><Pencil size={15} /></button><button className="icon-button table-action danger-icon" type="button" onClick={() => deleteVillage(region.id, village.id, village.activeListings)} aria-label="حذف القرية"><Trash2 size={15} /></button></div></div>)}</div>}
            </article>;
          })}</div>
        </section>
