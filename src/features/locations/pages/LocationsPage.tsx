      ...
      <div className="location-layout">
        <section className="card tree-panel">
          ...
          <div className="tree-list">{regions.map((region) => {
            ...
            return <article className="tree-region" key={region.id}><div className="tree-region-row"><button className="tree-toggle" type="button" aria-expanded={isExpanded} aria-controls={isExpanded ? villagesId : undefined} onClick={() => setExpanded((ids) => ids.includes(region.id) ? ids.filter((id) => id !== region.id) : [...ids, region.id])}>{isExpanded ? <ChevronDown aria-hidden="true" size={18} /> : <ChevronLeft aria-hidden="true" size={18} />}<MapPin aria-hidden="true" size={18} /><span><strong>{region.name}</strong><small>{region.villages.length} قرى</small></span></button><div className="row-actions"><StatusBadge value={region.isActive ? "enabled" : "disabled"} /><AuthorizedButton resource="regions" action="edit" className="icon-button table-action" type="button" onClick={() => openEditor({ type: "region", id: region.id, name: region.name })} aria-label={`تعديل منطقة ${region.name}`}><Pencil aria-hidden="true" size={15} /></AuthorizedButton><AuthorizedButton resource="villages" action="create" className="icon-button table-action" type="button" onClick={() => openEditor({ type: "village", regionId: region.id })} aria-label={`إضافة قرية إلى ${region.name}`}><Plus aria-hidden="true" size={16} /></AuthorizedButton></div></div>
              {isExpanded && <div className="village-list" id={villagesId}>{region.villages.map((village, index) => <div className="village-row" key={village.id}><span className="order-index">{index + 1}</span><span className="village-name"><strong>{village.name}</strong><small>{village.activeListings} إعلان نشط</small></span><StatusBadge value={village.isActive ? "enabled" : "disabled"} /><div className="row-actions"><AuthorizedButton resource="villages" action="edit" className="icon-button table-action" type="button" onClick={() => openEditor({ type: "village", regionId: region.id, id: village.id, name: village.name })} aria-label={`تعديل قرية ${village.name}`}><Pencil aria-hidden="true" size={15} /></AuthorizedButton><AuthorizedButton resource="villages" action="delete" className="icon-button table-action danger-icon" type="button" onClick={() => deleteVillage(region.id, village.id, village.activeListings)} aria-label={`حذف قرية ${village.name}`}><Trash2 aria-hidden="true" size={15} /></AuthorizedButton></div></div>)}</div>}
            </article>;
          })}</div>
        </section>
