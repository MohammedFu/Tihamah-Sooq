...
- **Completion note (2026-09-09):** Attached end-to-end correlation tracking (`X-Correlation-Id`, `X-Request-Id`) to `ApiClient` with automatic cryptographically random UUID generation, server header precedence, and diagnostic retention across network, timeout, HTTP, and envelope failures. `notificationStore.trackPromise` and `SystemPage` expose `requestId` directly in error toast descriptions (`معرّف العملية: {requestId}`) and error banners for operator diagnostics. Extended `RequestContext` across all admin services to propagate correlation IDs. Connected `getActingAdmin` to authenticated session storage (`adminSessionRepository.load()?.admin`) and established immutable audit trails committed strictly *after* mutation confirmation across users (ban), listings (status, soft-delete), commissions (verify), reports (resolve), broadcasts, and settings (general, batch, OTP, SMS). Enhanced `/system` audit table with formatted metadata details (reasons, review notes, broadcast titles, modified keys) and real-time text search. Remote audit adapter preserves fail-closed behavior (`UNCONFIRMED_ADMIN_CONTRACT`) as the remote backend does not register `GET /api/v1/admin/audit-logs` (Contract Gap 5). Documented in `docs/AUDIT_METADATA.md`. HTTP correlation tests, notification toast attribution tests, mutation audit recording tests, and system audit search tests passed cleanly (47 test files, 267 Vitest tests, 0 lint errors, clean production build).
