...
The dashboard statistics, listing moderation, and confirmed system-operation workflows now read through the registered provider. Other operational pages retain their task-specific integration status in `IMPLEMENTATION_PLAN.md`. The provider fixture store is separate from legacy page fixtures, lives in memory, resets on reload, and makes no network requests. Each factory invocation creates independent state.
...
- `settings.list/update/updateBatch/setOtpEnabled` use the documented settings paths and verbs. `settings.getSms/updateSms` use GET/PUT `/admin/settings/sms`; the mapper never exposes a returned API key and a blank editor value omits `sms_api_key` from updates. Single/batch/OTP mutations can return top-level acknowledgement fields rather than a `data` envelope. Setting keys cannot select nested routes. The SMS test remains disabled because `TestSMSRequest` has no confirmed body. See [the system operations guide](./SYSTEM_OPERATIONS.md).
